...
def prepare_scene():
    ...
    scene.cycles.samples = 128 # TODO: Tăng samples => Tăng chất lượng ảnh, giảm nhiễu | Tăng thời gian render
    scene.cycles.use_square_samples = False
    scene.display_settings.display_device = 'sRGB' # TODO: 
...
def prepare_rendersettings():
    bpy.ops.object.select_all(action='DESELECT')
    bpy.data.scenes['Scene'].cycles.device = 'GPU' # TODO: CPU / GPU
    bpy.data.scenes['Scene'].render.resolution_x = 448 # TODO
    bpy.data.scenes['Scene'].render.resolution_y = 448 # TODO
    bpy.data.scenes['Scene'].render.resolution_percentage = 100 # TODO
...
def add_lighting():
    ...
    if random.random() > 0.3:
        ...
        envp = envlist[idx]
        envnode.image = bpy.data.images.load(os.getcwd() + envp[0][1:])
        envstr = int(envp[1])
...
def page_texturing(mesh, texpath):
    ...
    texture_node.image = bpy.data.images.load(os.getcwd() + texpath[1:])
...
path_to_output_images = os.getcwd() + '/img/{}/'.format(rridx)
path_to_output_uv = os.getcwd() + '/uv/{}/'.format(rridx)
path_to_output_wc = os.getcwd() + '/wc/{}/'.format(rridx)
if save_blend_file:
